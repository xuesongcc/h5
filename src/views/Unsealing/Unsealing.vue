  ...
  export default {
    ...
    methods: {
      ...
      _submitCheck() {
        ...
        if (this.account === '') {
          this.$my_message('请输入您需要解封的手机号码');
          return;
        }
        if (!myreg.test(this.account)) {
          this.$my_message('需要解封的手机号码不合法，请重新输入');
          return;
        }
        if (this.password === '') {
      ...
      _submit(params) {
        ...
        this.isPoint = true;
        this.$FDPost('unblockAccount', params).then(re => {
          this.isPoint = false;
          if (re.status === 0) {
            this.$my_message(`用户${this.account}账户解封成功`);
            this.$router.back(-1);
          } else {
...
<template>
    <div class="unsealing">
        <BaseNavbar title="账户解封"></BaseNavbar>
        <div class="wrapper">
            ...
            <div class="item">
                <BaseInput id="account" name="解封账号" type="tel" :isNumber="isNumber" v-model="account" maxlength="11"
                           placeholder="请输入您需要解封的手机号码"></BaseInput>
            </div>
            <div class="item">
